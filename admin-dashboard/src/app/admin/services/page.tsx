...
export default function AdminServicesPage() {
  ...
  const [formData, setFormData] = useState({
    name: '', code: '', description: '', type: 'SERVICE', 
  ...
  const fetchProducts = async () => {
    ...
    try {
      const res = await api.get('/business-core/products', {
        params: { page, pageSize, search, type: 'SERVICE' }
      })
      if (res.data.items) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 border-b border-slate-200/80">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight text-slate-900 mb-1">Services</h1>
          <p className="text-slate-500">Manage your service offerings and pricing.</p>
        </div>
      ...
      <Modal
        ...
        title={editingProduct ? "Edit Service" : "New Service"}
        footer={
          <>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          <div>
            <label className="text-xs font-semibold text-slate-700 uppercase">Service Name</label>
              ...
              {variants.map((variant, index) => (
                <div key={index} className="flex items-center gap-3 bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                  <div className="flex-1">
                    ...
                    }} type="text" className="control-input w-full h-8 mt-1 text-sm" placeholder="e.g. Standard / Premium" />
                  </div>
                  <div className="w-24">
