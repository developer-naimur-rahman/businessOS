...
export default function AdminServiceCategoriesPage() {
  ...
  const fetchCategories = async () => {
    try {
      const res = await api.get('/business-core/categories?type=SERVICE')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const dataToSave = { ...formData, type: 'SERVICE' }
      if (editingCategory) {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex justify-between items-center bg-slate-900/50 p-6 rounded-3xl border border-slate-800 backdrop-blur-xl">
        <div>
          ...
          <p className="text-slate-400 mt-2">Manage your service catalog structure.</p>
        </div>
