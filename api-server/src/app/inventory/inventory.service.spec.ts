...
describe('InventoryService', () => {
  ...
  beforeEach(async () => {
    ...
    variantsService = {
      findOne: jest.fn().mockResolvedValue({ productId: 'prod-1' }),
    } as any;
  ...
  describe('createAdjustment', () => {
    it('should throw if product is a SERVICE', async () => {
      ...
      await expect(service.createAdjustment('org-1', { warehouseId: 'w-1', variantId: 'p-1', quantity: 10, type: 'ADJUSTMENT_IN' }, 'u-1'))
    ...
    it('should return existing movement on idempotent retry', async () => {
      ...
      const res = await service.createAdjustment('org-1', { warehouseId: 'w-1', variantId: 'p-1', quantity: 10, type: 'ADJUSTMENT_IN', idempotencyKey: 'abc' }, 'u-1');
      expect(res).toEqual(existing);
      expect(productsService.findOne).not.toHaveBeenCalled(); // Validations skipped
    });

    it('should throw if quantity is <= 0', async () => {
      await expect(service.createAdjustment('org-1', { warehouseId: 'w-1', variantId: 'p-1', quantity: 0, type: 'ADJUSTMENT_IN' }, 'u-1'))
        .rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('createTransfer', () => {
    it('should throw if source and dest warehouse are the same', async () => {
      await expect(service.createTransfer('org-1', { sourceWarehouseId: 'w-1', destinationWarehouseId: 'w-1', variantId: 'p-1', quantity: 10 }, 'u-1'))
        .rejects.toThrow(BadRequestException);
    });
    ...
    it('should return existing movements on idempotent retry', async () => {
      ...
      const res = await service.createTransfer('org-1', { sourceWarehouseId: 'w-1', destinationWarehouseId: 'w-2', variantId: 'p-1', quantity: 10, idempotencyKey: 'tx1' }, 'u-1');
